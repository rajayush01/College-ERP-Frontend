          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <Button
              onClick={async () => {
                try {
                  setLoading(true);
                  const response = await fetch('/api/admin/fee-records/update-statuses', {
                    method: 'POST',
                    headers: {
            ...
            <Button
              onClick={async () => {
                try {
                  setLoading(true);
                  const response = await fetch('/api/admin/fee-records/fix-indexes', {
                    method: 'POST',
                    headers: {
                      'Authorization': `Bearer ${localStorage.getItem('token')}`,
                      'Content-Type': 'application/json'
                  ...
                  alert('Failed to fix database indexes');
                } finally {
                  setLoading(false);
                }
              }}
            ...
            <Button
              onClick={async () => {
                if (confirm('⚠️ DANGER: This will delete ALL fee records! Are you sure?')) {
                  try {
                    setLoading(true);
                    const response = await fetch('/api/admin/fee-records/reset-collection', {
                      method: 'POST',
                      headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                        'Content-Type': 'application/json'
                    ...
                    console.error('Failed to reset fee records:', error);
                    alert('Failed to reset fee records');
                  } finally {
                    setLoading(false);
                  }
                }
            ...
            <Button
              onClick={async () => {
                try {
                  setLoading(true);
                  const response = await fetch('/api/admin/fee-records/create-missing', {
                    method: 'POST',
                    headers: {
                      'Authorization': `Bearer ${localStorage.getItem('token')}`,
                      'Content-Type': 'application/json'
                  ...
                  console.error('Failed to create missing fee records:', error);
                  alert('Failed to create missing fee records');
                } finally {
                  setLoading(false);
                }
              }}
            ...
            <Button
              onClick={async () => {
                try {
                  setLoading(true);
                  const response = await fetch('/api/admin/fee-records/debug-statistics', {
                    method: 'GET',
                    headers: {
                      'Authorization': `Bearer ${localStorage.getItem('token')}`,
                      'Content-Type': 'application/json'
                  ...
                  const result = await response.json();
                  console.log('Debug statistics result:', result);
                  alert(`Debug Statistics:\nFee Records: ${result.basicCounts?.totalFeeRecords}\nStudents: ${result.basicCounts?.totalStudents}\nCurrent Year Records: ${result.currentYearRecords}`);
                } catch (error) {
                  console.error('Failed to get debug statistics:', error);
                  alert('Failed to get debug statistics');
                } finally {
                  setLoading(false);
                }
              }}
